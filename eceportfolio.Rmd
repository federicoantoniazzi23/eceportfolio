...
```{r}
# Create the histogram plot with tempo instead of key without mode legend
ggplot(ecedisco, aes(x = tempo)) +
  geom_tile(stat = "count", color = "black", size = 0.5, aes(fill = as.factor(mode))) +
  facet_wrap(~category) +
  labs(x = "Tempo", y = "Count") +  # Change x-axis label to "Tempo"
  scale_x_continuous(breaks = seq(0, max(ecedisco$tempo), by = 10)) +  # Adjust breaks for tempo
...
stomaco_plot <- stomaco |> 
  ...
  theme(axis.text.y = element_text(size = 6))  # Imposta la dimensione del testo dell'asse y
